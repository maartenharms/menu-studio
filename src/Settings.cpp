...
namespace {
    ...
    // Background + stage presets - independent axes (r18, user design:
    // "mix and match"). Paths verified against the vanilla BSAs
    // (Tools\re\bsa_list.py). Field history: r16's borrowed-architecture
    // stages and r17's brazier ring both rejected - starlight's dais is
    // the proven floor; proper rooms arrive with the layout import (F-2).
    // r57: the shell is now an OPAQUE lighting sphere - it renders in the
    // opaque pass and occludes the sky for ANY dome layered in front of it,
    // so the two-surface sort conflict that convicted every non-default dome
    // (r30/r47/r48) is GONE. The background is a free menu of the game's
    // constellation domes, each backed by the void shell. Paths verified in
    // the vanilla BSAs (tools/bsa_list.py); a mesh the load order lacks just
    // logs "demand failed" and the dome is skipped (the shell stays), so DLC/
    // mod-gated entries can't crash. Default size/height = the r57 field
    // finding (2000 / 0); the panel sliders retune per dome. The old −250
    // sink (to hide the perk dome's hemisphere rim) is unneeded now that the
    // opaque shell owns the whole backdrop.
    constexpr std::array<MTB::BackgroundPreset, 5> kBackgroundPresets{ {
        // r59: 'blank' is a flat VOID COLOUR - an effect sphere (voidcolor.nif)
        // the picker/vibe tints, sitting inside the opaque shell (which blocks
        // ENB sky). The lighting shell alone renders pure black (its flat
        // emissive never showed); the effect baseColor tint is how the colour
        // actually paints. Disabling the shell instead would re-expose the ENB
        // daytime sky, so we keep it and layer the colour on top.
        { "blank", "mtb\\voidcolor.nif", 2000.0f, 0.0f },
        { "constellation", "interface\\intperkskydome.nif", 2000.0f, 0.0f },
        // Dawnguard's vampire perk tree - a different constellation (the wolf
        // glyphs the r47 note flagged are the LOOK here, not a bug). Needs the
        // Dawnguard DLC.
        { "vampire", "dlc01\\interface\\intvampireperkskydome.nif", 2000.0f, 0.0f },
        // The third vanilla perk dome (ships in Skyrim - Meshes1).
        { "teat", "interface\\teatperkskydome.nif", 2000.0f, 0.0f },
        // r58: CUSTOM IMAGE - our own opaque textured sphere (glow map on the
        // emissive) instead of a vanilla dome. Wraps textures\mtb\voidshell_g.dds
        // (ships an example nebula; drop your own 2:1 equirectangular image over
        // it) shown AS AUTHORED (Backdrop leaves this one untinted). "Lock
        // background angle" frames it. (r57 'aurora' dropped: the additive
        // curtain rendered pure black under the dark void tint - reintroduce
        // with a no-tint path if auroras are wanted.)
        { "custom", "mtb\\voidimage.nif", 2000.0f, 0.0f },
    } };

    constexpr std::array<MTB::StagePreset, 1> kStagePresets{ {
        { "starlight", "clutter\\nightingale\\nightingaleplatform.nif", 600.0f, -10.0f, {} },
    } };

    const MTB::StagePreset* FindStage(std::string_view a_name) {
        for (const auto& stage : kStagePresets) {
            if (a_name == stage.name) {
                return &stage;
            }
        }
        return nullptr;
    ...
    const MTB::BackgroundPreset* FindBackground(std::string_view a_name) {
        for (const auto& bg : kBackgroundPresets) {
            if (a_name == bg.name) {
                return &bg;
            }
        }
