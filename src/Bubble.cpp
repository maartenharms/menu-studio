...
namespace MTB {
    ...
    void Bubble::OnFrame(RE::Main* a_main) {
        ...
        // Per-frame reconcile: if force-pause was switched off live (FLICK
        // panel) while we still hold a pause, release it now so the open menu
        // un-freezes immediately. Runs regardless of pause state.
        ForcePause::Reassert();
